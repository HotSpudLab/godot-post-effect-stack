...
func _render_callback(p_effect_callback_type: EffectCallbackType, p_render_data: RenderData) -> void:
	...
	# Multi-pass effects build their own pipelines and bindings, so they must be
	# handed the buffers directly rather than run through the single-dispatch
	# path below. `EffectStackRunner` already did this; without it here, a
	# multi-pass effect assigned to this runner would either do nothing (no
	# `shader_file`, e.g. Bloom) or dispatch with the wrong bindings.
	if effect_resource._is_multi_pass:
		for view in range(render_scene_buffers.get_view_count()):
			effect_resource._render_multi_pass(rd, self, render_scene_buffers, view, size, render_scene_buffers.get_color_layer(view))
		return
