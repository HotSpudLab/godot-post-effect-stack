...
## Bind a copy of the colour buffer, taken before this effect runs, as a
## `sampler2D`. Set this if the effect reads any pixel other than the one it
## writes: effects run in place, so reading `color_image` at an offset races
## against the workgroups writing it. See the binding table in
## `addons/post_effect_stack/effect_dispatch.gd`.
@export var needs_source_snapshot: bool = false
