...
|---|---|---|
| `addons/post_effect_resource/` | Required | The engine itself (Resource base class, Runner, shader template). Contains zero effects |
...
1. Copy both `addons/post_effect_resource/` (the engine) and `effects/` (the bundled effect collection) into your project, and enable the plugin
...
1. `<name>.gd` — extends `PostEffectResource` and builds the parameters passed to the shader in `_build_push_constant()`
2. `<name>.glsl` — a compute shader written using the `#COMPUTE_CODE` substitution in `addons/post_effect_resource/shaders/template_post_effect.glsl`
...
To provide a canvas path for Mobile/Compatibility, add a `<name>_canvas.gdshader` (based on `hint_screen_texture`) and assign it to `effect_material` on `addons/post_effect_resource/canvas_post_effect.tscn`. Depth-dependent effects can't provide a canvas path.
...
A `CompositorEffect` that runs a single `PostEffectResource`. Lower overhead than `EffectStackRunner` for single-effect use.

### `EffectStackResource` (`addons/post_effect_resource/effect_stack_resource.gd`)

A Resource holding `effects: Array[PostEffectResource]`. Supports reordering, adding, and removing in the Inspector.
