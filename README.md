...
**2. Sampling any pixel other than your own needs a snapshot.** Effects run in place on the colour buffer, so reading a neighbour is not safe — see the note in the effect authoring guide above. Also note that `imageLoad()` takes integer coordinates and does no filtering, so there is no free bilinear sample: shaders that rely on smooth UV sampling (distortion, blur, shockwave, chromatic aberration) need a snapshot pass and their own interpolation.
